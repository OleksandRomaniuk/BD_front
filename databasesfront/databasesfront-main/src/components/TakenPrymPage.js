...
function TakenPrymPage({token}){
    const [requests, setRequests] = useState(false)

    useEffect(() => {
        const data = fetch("http://localhost:8090/api/Cooperation/allTakenCoopNotExpired",{
        method: 'GET',
        headers: {
        ...
        for(let r of requests){
            requests_arr.push(
                <tr>
                    ...
                    <td><Button onClick={() => approve(r.id)}>Підтвердити повернення</Button></td>
                </tr>
            )
        }
